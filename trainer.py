...
# ---------- Helper: make dataloader sampler ----------
def make_weighted_sampler(labels, num_samples=None):
...
def train_model(model, train_loader, val_loader, fold=0, device=None):
    ...
    class_weights = compute_class_weight('balanced', classes=classes, y=all_train_labels)
    class_weights = torch.tensor(class_weights, dtype=torch.float32).to(device)
    print(f"→ Class weights: {class_weights.cpu().numpy()}")

    # Replace loss with FocalLoss
    criterion = FocalLoss(weight=class_weights, gamma=2.0).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        # ---------- Validation ----------
        val_loss, val_bal_acc, _, _, val_f1, val_preds, val_targets, _ = validate_model_with_metrics(model, val_loader, criterion, device)
        ...
        print(f"Epoch {epoch+1}/{args.epochs} | "
              ...
              f"Train Acc: {train_bal_acc:.3f} | Val Acc: {val_bal_acc:.3f} | "
              f"Train F1: {train_f1:.3f} | Val F1: {val_f1:.3f} | "
              f"Val MCC: {val_mcc:.3f}")
        ...
        if val_bal_acc > best_bal_acc:
            ...
            print(f"✅ Best model saved (Balanced Acc = {best_bal_acc:.4f})")
        else:
            no_improve_epochs += 1
    ...
    # ---------- Final Report ----------
    print(f"\nFold {fold} finished. Best Balanced Accuracy: {best_bal_acc:.4f}")
    pd.DataFrame({
        'epoch': np.arange(len(train_losses)),
        'train_loss': train_losses,
        'val_loss': val_losses,
        'train_bal_acc': train_bal_accs,
        'val_bal_acc': val_bal_accs,
        'train_f1': train_f1_scores,
        'val_f1': val_f1_scores,
        'train_mcc': train_mccs,
        'val_mcc': val_mccs
    }).to_csv(os.path.join(args.out_dir, f"fold_{fold}_metrics.csv"), index=False)

    return best_bal_acc


def validate_model_with_metrics(model, val_loader, criterion, device):
    model.eval()
    if device is None:
    ...
    return val_loss, val_bal_acc, None, None, val_f1, all_preds, all_targets, all_probs
